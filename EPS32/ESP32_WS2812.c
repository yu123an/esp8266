...
int WiFiFlag = 0;                    //联网标志位，1：联网，0：未联网
int light;
...
void setup() {
  ...
  WS.begin();  // WS2812驱动使能
  Serial.begin(9600);
  if (i2ceeprom.begin(0x50)) {  // EEProm使能
  ...
  if (now.Second() == 33) {
    for (int mm = 0; mm < 10; mm++) {
      ...
      for (int num = 0; num < num_gif; num++) {
        for (int i = 0; i < 64; i++) {
          WS.setPixelColor(128 + i, i2ceeprom.read(num * 192 + 2 + i * 3), i2ceeprom.read(num * 192 + 2 + i * 3 + 1), i2ceeprom.read(num * 192 + 2 + i * 3 + 2));
        }
        WS.show();
        delay(1000 / num_gif);
      }
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (type == "time") {
    ...
    //存储gif帧数据
    i2ceeprom.write(0, Mqtt_Sub["totle"]);
    int num = Mqtt_Sub["in"];
    for (int i = 0; i < 192; i++) {
      i2ceeprom.write(num * 192 + i + 2, Mqtt_Sub["bmp"][i]["n"]);
    }
  } else if (type == "Find") {
    int addr = Mqtt_Sub["addr"];
    int num = Mqtt_Sub["num"];
    for (int i = 0; i < num; i++) {
      Serial.print("what you find address is : ");
      Serial.print(i);
      Serial.print(" ;The data is : ");
      Serial.println(i2ceeprom.read(i));
    }
  } else if (type == "PlayGif") {
    int num_gif = i2ceeprom.read(0);
    for (int num = 0; num < num_gif; num++) {
      for (int i = 0; i < 64; i++) {
        WS.setPixelColor(i, i2ceeprom.read(num * 192 + 2 + i * 3), i2ceeprom.read(num * 192 + 2 + i * 3 + 1), i2ceeprom.read(num * 192 + 2 + i * 3 + 2));
      }
      WS.show();
      delay(1000 / num_gif);
    }
...
void Pub_msg() {
  ...
  String I = ":";
  String G = "}";
  String ALL = A + String(Temp_in) + B + String(Humidity_in) + C + String(now.Hour()) + I + String(now.Minute()) + E + String(Sun_rise_hour) + I + String(Sun_rise_minute) + H + String(Sun_set_hour) + I + String(Sun_set_minute) + F + String(light) + G;
  char _ALL[800];
